...
Datasheet.schema.pre('save', function (next) {
	var self = this;
	...
	.exec(function(err, existing) {
		if(err) {
			console.log('ERROR: Datasheet - There was an error finding the datasheet during pre save.');
		}
		if(existing) {
...
Datasheet.relationship({ ref: 'Microcontroller', path: 'microcontrollers', refPath: 'datasheets' });

Datasheet.defaultColumns = 'title|20%, service|20%, createdBy|20%, state|20%';
Datasheet.register();
